...
powershell -NoProfile -ExecutionPolicy Bypass -Command "if(@('0','false','no','off') -contains ([string]$env:EVEJS_CLIENT_SAFE_WINDOWED).Trim().ToLowerInvariant()){Write-Host 'Client display safety reset disabled by EVEJS_CLIENT_SAFE_WINDOWED.'; exit 0}; $p=[IO.Path]::GetFullPath($env:EVEJS_CLIENT_PATH).TrimEnd('\','/'); $k=$p.ToLowerInvariant() -replace ':','' -replace '[\\/]+','_' -replace '[^a-z0-9._-]+','_'; $k=$k.Trim('_') + '_127.0.0.1'; $d=Join-Path $env:LOCALAPPDATA ('CCP\EVE\' + $k + '\settings'); New-Item -ItemType Directory -Force -Path $d | Out-Null; $f=Join-Path $d 'core_public__.yaml'; if((Test-Path -LiteralPath $f) -and -not (Test-Path -LiteralPath ($f + '.evejs-display-backup'))){Copy-Item -LiteralPath $f -Destination ($f + '.evejs-display-backup')}; try{Add-Type -AssemblyName System.Windows.Forms -ErrorAction Stop; $a=[Windows.Forms.Screen]::PrimaryScreen.WorkingArea; $sw=[int]$a.Width; $sh=[int]$a.Height}catch{$sw=1600; $sh=900}; $w=[Math]::Min(1600,[Math]::Max(800,$sw-80)); $h=[Math]::Min(900,[Math]::Max(600,$sh-120)); $cw=0; if([int]::TryParse([string]$env:EVEJS_CLIENT_WINDOW_WIDTH,[ref]$cw) -and $cw -gt 0){$w=[Math]::Min($cw,$sw)}; $ch=0; if([int]::TryParse([string]$env:EVEJS_CLIENT_WINDOW_HEIGHT,[ref]$ch) -and $ch -gt 0){$h=[Math]::Min($ch,$sh)}; $t=[DateTime]::UtcNow.ToFileTimeUtc(); $lines=@('device:', '  DeviceSettings: ['+$t+', null]', '  FixedWindow: ['+$t+', 0]', '  FixedWindowSettings: ['+$t+', null]', '  FullScreenResolution: ['+$t+', null]', '  FullScreenSettings:', '  - '+$t, '  - {adapter: 0, height: '+$sh+', presentInterval: 1, width: '+$sw+'}', '  UIScaleFullscreen: ['+$t+', 1.0]', '  UIScaleFullscreenSetAutomatically: ['+$t+', true]', '  UIScaleWindowed: ['+$t+', 1.0]', '  UIScaleWindowedSetAutomatically: ['+$t+', false]', '  WindowMode: ['+$t+', 1]', '  WindowedResolution: ['+$t+', null]', '  WindowedSettings:', '  - '+$t, '  - {adapter: 0, height: '+$h+', left: 0, presentInterval: 1, showState: 1, top: 0, width: '+$w+'}', 'generic: {}', 'ui: {}'); Set-Content -LiteralPath $f -Value $lines -Encoding UTF8; Write-Host ('Client display safety reset applied: windowed {0}x{1} at 0,0.' -f $w,$h); Write-Host ('Settings: ' + $f)"
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "if(@('0','false','no','off') -contains ([string]$env:EVEJS_CLIENT_SAFE_GRAPHICS).Trim().ToLowerInvariant()){Write-Host 'Client graphics safety reset disabled by EVEJS_CLIENT_SAFE_GRAPHICS.'; exit 0}; $p=[IO.Path]::GetFullPath($env:EVEJS_CLIENT_PATH).TrimEnd('\','/'); $k=$p.ToLowerInvariant() -replace ':','' -replace '[\\/]+','_' -replace '[^a-z0-9._-]+','_'; $k=$k.Trim('_') + '_127.0.0.1'; $f=Join-Path $env:LOCALAPPDATA ('CCP\EVE\' + $k + '\settings\core_public__.yaml'); if(-not (Test-Path -LiteralPath $f)){exit 0}; $t=[DateTime]::UtcNow.ToFileTimeUtc(); $raw=Get-Content -LiteralPath $f; $keys=@('  antiAliasing: ['+$t+', 0]','  aoQuality: ['+$t+', 0]','  charClothSimulation: ['+$t+', 0]','  dofEnabled: ['+$t+', false]','  frameGeneration: ['+$t+', false]','  lodQuality: ['+$t+', 1]','  postProcessingQuality: ['+$t+', 0]','  reflectionQuality: ['+$t+', 0]','  shaderQuality: ['+$t+', 1]','  shadowQuality: ['+$t+', 0]','  upscalingTechnique: ['+$t+', 0]','  volumetricQuality: ['+$t+', 0]'); $out=New-Object System.Collections.Generic.List[string]; foreach($line in $raw){ if($line -match '^  (antiAliasing|aoQuality|charClothSimulation|dofEnabled|frameGeneration|lodQuality|postProcessingQuality|reflectionQuality|shaderQuality|shadowQuality|upscalingTechnique|volumetricQuality):'){ continue }; if($line -eq 'generic: {}'){ foreach($key in $keys){$out.Add($key)} }; $out.Add($line) }; Set-Content -LiteralPath $f -Value $out -Encoding UTF8; Write-Host 'Client graphics safety reset applied: low GPU profile.'"
exit /b %errorlevel%
